...
parser <- ArgumentParser(description = 'Filter probes/CpG sites--quality control')

parser$add_argument('--probe_fol', 
                    default = '../../data/probes/', 
                    help = 'Relative path to where probe information is stored')

parser$add_argument('--methylation_fol', 
                    default = '../../data/', 
                    help = 'Relative path to where methylation data is stored')

parser$add_argument('--MAF_cutoff', default = 0.05, 
                    help = 'Minor allele frequency threshold point')

parser$add_argument('--detectP_cutoff', default = 0.01, 
                    help = 'Detection p-value threshold point')
...
ofile <- paste0(args$probe_fol, "probe-fate.csv")


M.file <- paste0(args$methylation_fol, "/M.csv")
M2.file <- paste0(args$methylation_fol, "/M-filtered.csv")

beta.file <- paste0(args$methylation_fol, "/beta.csv")
beta2.file <- paste0(args$methylation_fol, "/beta-filtered.csv")
